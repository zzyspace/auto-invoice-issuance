...
- `status.json`：运行 PID、最后更新时间、当前步骤、最后日志/异常、最终状态；每 15 秒写一次心跳，
  ...
  任意步骤持续 30 秒以上时，心跳还会写出业务线程的 `step_stack`（文件、函数、行号），
  覆盖没有 AX 调用的浏览器/子进程等待；长时间等待本身不被标记成失败。
- AX 调用单独记录 `current_operation`：目标 PID、控件引用、访问数量、属性或动作、开始时间、
  持续时间和最近 12 次调用。快调用只留内存记录，避免大量属性查询淹没日志；单次调用持续
  2 秒以上时，后台自动写 `operation.blocked` 和所属线程的 Python 文件/函数/行号，
  后续最多每 10 秒补一次，不依赖业务线程主动报错。不记录控件值、验证码或局部变量。
...
App 登录进一步区分 `app_submit_credentials`、`app_submit_sms_login`、
`app_wait_login_result`，避免把点击登录和等候弹窗都显示成 `app_login`。
Playwright 页面自动记录跳转和网络错误；切换到 raw CDP 后保留步骤日志，失败时采集当前页面
状态及最近请求状态摘要，不额外建立长期调试连接。

排查时先看 `status.json`，再看同目录的 `events.jsonl`。`Ctrl+C` 会尽量保存中断堆栈，
并按原逻辑抛出中断；调试不会额外写入业务历史或改写信号处理。`SIGTERM`、`SIGHUP`、
强制杀进程（`SIGKILL`）、断电等可能无法写结束记录，
此时应结合最后心跳和 PID 是否存活判断，不能仅凭 `running` 认为还在运行。
尚未读完工作簿或被中断时，仅诊断文件记录新增证据；业务状态和历史沿用原流程。
细分步骤和准确失败位置以本次 `status.json` / `events.jsonl` 为准，原业务表不额外细化。

AX 调试仅观察原有调用，原始返回值、异常、遍历方式、引用管理和点击/输入兜底均保持原行为。
不设置新的 AX 超时、扫描时间/节点/深度上限，不增加启动重试或新的终止条件。
`ax.scan.*` 记录访问数量和耗时，`ax.call.returned_error` 记录原生错误码，
但错误码如何处理仍由原业务逻辑决定。故障采集只使用已缓存信息，不额外扫描 App。
后台线程只记录调用栈和进度，不取消调用，也不控制运行流程。
更改代码后需重新启动 runner 才会使用新逻辑，既有进程不会热更新。
...
- CSV 里的税号会先做字母数字归一化，英文统一转大写
- 中文说明文字，例如 `无税号 个人抬头`，会被视为空
- 企业抬头税号为空，或长度/格式不符合常见企业税号规则（`15` 位纯数字或 `18` 位统一社会信用代码）时，会尝试调外部查询接口
