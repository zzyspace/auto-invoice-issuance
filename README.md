...
- AX 调用单独记录 `current_operation`：目标 PID、控件引用/树路径、属性或动作、开始时间、
...
AX 界面读取使用 [macOS 原生消息超时](https://developer.apple.com/documentation/applicationservices/1459345-axuielementsetmessagingtimeout)：单次最多 1 秒；一次完整扫描最多 5 秒、800 个控件、
64 层深度（外层操作剩余时间更短时取更短值），同时限制保留的引用数量。
通过 `CFHash` / `CFEqual` 识别指向同一控件的不同原生引用，阻止重复遍历；原生引用随
结果节点的生命周期释放。`ax.scan.*` 保留访问数量、重复数量、深度、路径和耗时，便于区分
单个属性通信失败与控件树遍历异常。

AX 超时或扫描超限会停止当前门店及后续门店，不进入手动登录等待，也不在结果不明时重复点击。
故障采集只使用已缓存的扫描信息，避免再次调用同一个失去响应的界面读取。
后台线程提供定位证据，原生消息超时和扫描预算负责退出；后台诊断本身不负责取消原生调用。
